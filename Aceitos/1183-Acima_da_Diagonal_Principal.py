...
# mostre a soma ou a média considerando somente aqueles elementos que estão acima da diagonal principal da matriz, conforme ilustrado abaixo (área verde).
...
def main() -> None:
    ...
    for x in range(tamanho_da_matriz):
        for y in range(tamanho_da_matriz):
            if y > x:
                soma += matriz[x][y]
