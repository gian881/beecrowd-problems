...
def main() -> None:
    while True:
        try:
            n_pessoas, n_datas = map(int, input().split())
            data_final = 'Pizza antes de FdI'
            for _ in range(n_datas):
                data, *pessoas = input().split()
                if pessoas.count('1') == n_pessoas and data_final == 'Pizza antes de FdI':
                    data_final = data
            print(data_final)
        except EOFError:
            break
