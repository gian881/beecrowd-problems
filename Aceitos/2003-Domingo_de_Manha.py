...
def main() -> None:
    hora_encontro: datetime = datetime(2021, 8, 10, 8, 0, 0)
    while True:
        try:
            hora_acordada: datetime = datetime.strptime(
                f'10/08/21 0{input()}:00', '%d/%m/%y %H:%M:%S')

            hora_maxima: datetime = hora_acordada + timedelta(hours=1)

            if hora_maxima > hora_encontro:
                print(
                    f'Atraso maximo: {(hora_maxima - hora_encontro).seconds//60}')
            else:
